Feature: User Table Page table data check.
  ...
  And Description text under image in '<Number>' row should be displayed as '<Description>' with '<Additional description>'
  And Checkboxes should be displayed in '<Number>' row at Users Table on User Table Page
    Examples:
      | Number |Username          |Description                       |Additional description|
      | 1      | Roman            | Wolverine                        |                      |
  ...
  Scenario: User Table Page dropdown test
    Given I open JDI main page
    And I login as user Roman Iovlev
    When I click on 'Service' button in Header
    And I click on 'User Table' button in Service dropdown
    Then User Table page should be opened
    And Dropdown should have next options for user 'Roman' at Users Table
    |Admin  |
    |User   |
    |Manager|